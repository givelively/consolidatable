...
RSpec.describe Consolidatable::ConsolidationFetcher, db_access: true do
  ...
  let(:fetcher) do
    described_class.new(owner, var_name: var_name,
                               var_type: var_type,
                               computer: computer,
  ...
  before do
    Child.send(:consolidates, computer, type: var_type)
  end
  ...
  context 'when no matching Consolidation can be found' do
    ...
    it 'creates a new Consolidation' do
      expect { call }
        .to change(owner.consolidations, :count)
        .from(0).to(1)
    end
  end
  ...
  context 'when given an object with eager loaded Consolidations' do
    ...
    it 'does not make any AR calls' do
      ...
      expect(log_queries { owner.send(var_name) })
        .not_to include('Consolidatable::Consolidation Load')
    end
  end
end
